...
export const ShipComponent = (name, width, length, board) => {
  ...
  shipDraggable.resetRotation = () => {
    if (ship.position === "vertical") {
      generateShipDraggable(shipDraggable, true, board);
    }
  };

  pubsub.subscribe("changedGameState", (state) => {
    if (state === "placement") {
      shipDraggable.draggable = true;
    } else {
      shipDraggable.draggable = false;
    }
  });

  shipContainer.appendChild(shipDraggable);
  ship.shipDraggable = shipDraggable;
  generateShipDraggable(shipDraggable, false, board);

  const shipPlacementBg = document.createElement("div");
  shipPlacementBg.classList.add("ship-placement-bg");
  generateShipBlock(shipPlacementBg, ship.width, ship.length, board);
...
const generateShipBlock = (parentElement, width, length, board) => {
      ...
      if (x === 0 && y === 0 && parentElement.ship) {
        ...
        rotateButton.addEventListener("click", () => {
          if (
            ...
            return;
          generateShipDraggable(parentElement, true, board);
        });

        // Show rotate button when dragged into board============================
        if (parentElement.ship.board) {
          if (
            parentElement.ship.board.shipNodes[parentElement.ship.name].nodes
              .length !== 0
          )
        ...
        parentElement.rotateButton = rotateButton;
      }
    }
  }
  const boardCell = board.board.cellElement;
  parentElement.style.gridTemplateColumns = `repeat(${width}, ${boardCell.offsetWidth}px)`;

  pubsub.subscribe("displayGameStatus", () => {
    parentElement.style.gridTemplateColumns = `repeat(${width}, ${boardCell.offsetWidth}px)`;
  });
...
export const generateShipDraggable = (shipDraggable, rotate = false, board) => {
  ...
  generateShipBlock(shipDraggable, ship.width, ship.length, board);
  ...
  shipDraggable.addEventListener("dragend", (e) => {
    ...
    if (ship.board.shipNodes[ship.name].nodes.length === 0) {
      shipDraggable.resetRotation();
      shipDraggable.rotateButton.classList.remove("show");
    } else {
      shipDraggable.rotateButton.classList.add("show");
    }
  });
